...
mini_batch_size = 20		# Geriausi (su ~200000 pvz): [20 - 64]
epochs = 200				# Treniruočių (iteracijų) skaičius
...
synapse_2 = 2*np.random.random((hidden_layer_1_size, output_size)) - 1
...
for i in range(0, expanded_inputs.shape[0]):
	...
	if(network_4_class_mode and np.argmax(layer_2) == np.argmax(input_outputs[i])):
		successes.append(1)
	elif((input_outputs[i] == 0 and layer_2 < 0.5) or (input_outputs[i] == 1 and layer_2 >= 0.5)):
		successes.append(1)
